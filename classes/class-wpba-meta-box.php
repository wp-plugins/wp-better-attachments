...
class WPBA_Meta_Box extends WP_Better_Attachments
{
	...
	public function render_meta_box_content() {
		global $post; ?>
		<div id="<?php echo esc_attr( "wpba-post-{$post->ID}" ); ?>" data-postid="<?php echo esc_attr( $post->ID ); ?>" class="clearfix wpba<?php echo esc_attr( $this->display_class() ); ?>">
			<input type="hidden" name="wpba_nonce" id="wpba_nonce" value="<?php echo esc_attr( wp_create_nonce( basename( __FILE__ ) ) );?>" />
			<div class="uploader pull-left">
				...
				<?php // <a class="button wpba-attachments-button" id="wpba_external_attachments_button" href="#"><span class="wpba-media-buttons-icon"></span> Add External Attachments</a> ?>
			</div>
			<div class="pull-left wpba-saving hide">
				<span>Saving Attachments </span>
				<img src="<?php echo esc_attr( admin_url( 'images/wpspin_light.gif' ) ); ?>">
			</div>
			<div class="clear"></div>
			<?php echo wp_kses( $this->output_post_attachments() ); ?>
			<div class="clear"></div>
		</div>
		<?php
		echo wp_kses( $this->edit_modal() );
		// echo wp_kses( $this->add_external_attachments_modal() );
	...
	/**
	 * Add external attachments modal
	 *
	 * @since 1.1.0
	 *
	 * @return string Edit modal HTML
	 */
	public function add_external_attachments_modal() {
		// Modal does not exist pre 3.5
		global $wp_version;
		if ( floatval( $wp_version ) <= 3.4 )
			return '';
		...
		$html  = '';
		$nl    = "\n";
		$html .= "<div tabindex='0' id='wpba_add_external_attachments_screen' class='supports-drag-drop' style='display: none;'>" . $nl;
		$html .= '<div class="media-modal wp-core-ui">' . $nl;
		$html .= '<a id="wpba_add_external_attachments_screen_close" class="media-modal-close" href="#" title="Close"><span class="media-modal-icon"></span></a>' . $nl;
		$html .= '<div class="media-modal-content">' . $this->add_external_attachments_modal_form() . '</div>' . $nl;
		$html .= '</div>' . $nl;
		$html .= '<div class="media-modal-backdrop"></div>' . $nl;
		$html .= '</div>' . $nl;

		return $html;
	} // add_external_attachments_modal()



	/**
	 * Add external attachments modal form
	 *
	 * @since 1.1.0
	 *
	 * @return string Edit modal HTML
	 */
	public function add_external_attachments_modal_form() {
		global $post;
		$inital_fields_count = 5;
		$html = '';
		$nl   = "\n";

		$html .= "<div id='add_external_attachments_modal_form' data-postid='{$post->ID}'>" . $nl;
		$html .= '<h1>Add External Attachments</h1>' . $nl;
		$html .= '<ul class="unstyled">' . $nl;
		for ( $i = 0;  $i < $inital_fields_count;  $i++ ) {
			$field_num = $i + 1;

			$html .= '<li>' . $nl;
			$html .= "<label for='wpba_external_attachment_url_{$field_num}''>Link:</label>" . $nl;
			$html .= "<input type='text' name='wpba_external_attachment_url_{$field_num}' id='wpba_external_attachment_url_{$field_num}' class='wpba-external-attachment-url' value='' placeholder='http://'>" . $nl;
			$html .= '<br>';
			$html .= "<label for='wpba_external_attachment_title_{$field_num}'>Title:</label>" . $nl;
			$html .= "<input type='text' name='wpba_external_attachment_title_{$field_num}' id='wpba_external_attachment_title_{$field_num}' class='wpba-external-attachment-title' value='' placeholder='title'>" . $nl;
			$html .= '<br>';
			$html .= $this->external_attachment_mime_type_select_list_html();
			$html .= '</li>' . $nl;
		} // for()
		$html .= '</ul>' . $nl;

		$html .= '<div id="add_external_attachments_footer">' . $nl;
		$html .= '<a href="#" id="add_external_attachments_submit" class="button media-button button-primary button-large media-button-select">Add External Attachment(s)</a>' . $nl;
		$html .= '</div>' . $nl;
		$html .= '</div>' . $nl;
		return $html;
	} // add_external_attachments_modal_form()



	/**
	 * External attachment mime type select list
	 * Allows user to select the mime type of the external resource
	 *
	 * @return string Select list HTML
	 */
	public function external_attachment_mime_type_select_list_html() {
		$html = '';
		$nl = "\n";
		$allowed_mime_types = get_allowed_mime_types();
		$allowed_mime_types['other'] = 'wpba/external';
		ksort( $allowed_mime_types );

		$html .= "<label for='wpba_external_attachment_mime_type_{$field_num}'>File Type:</label>" . $nl;
		$html .= "<select 'wpba_external_attachment_mime_type_{$field_num}' id='wpba_external_attachment_mime_type_{$field_num}' class='wpba-external-attachment-mime-type'>" . $nl;
		$html .= "<option value='' selected='selected'>Select File Type</option>" . $nl;
		foreach ( $allowed_mime_types as $allowed_mime_type_key => $allowed_mime_type_value ) {
			$html .= "<option value='{$allowed_mime_type_value}'>{$allowed_mime_type_key}</option>" . $nl;
		} // foreach()
		$html .= '</select>' . $nl;

		return $html;
	} // external_attachment_mime_type_select_list()
